<form action="{{ url('/create-sub-criteria') }}" method="POST" class="p-6 mx-6 bg-white mt-5">
    @csrf
    <div class="w-1/2">
        <h1 class="text-xl mb-4 font-semibold">Form Create Sub Criteria</h1>
        <select name="id_criteria" id="id_criteria"
            class="rounded border focus:outline-none focus:border-blue-500 p-2 text-base w-full mb-3 {{ $errors->has('id_criteria') ? 'border-red-500' : 'border-gray-300' }}"
            required>
        ...
        <input name="sub_criteria" id="sub_criteria"
            type="text"
            class="rounded border focus:outline-none focus:border-blue-500 p-2 text-base w-full mb-3 {{ $errors->has('sub_criteria') ? 'border-red-500' : 'border-gray-300' }}"
