<form action="{{ url('/update-criteria/' . $criteria->id) }}" method="POST" class="p-6 mx-6 bg-white mt-5">
    @csrf
    @method('PUT')
    <div class="w-1/2">
        <h1 class="text-xl mb-4 font-semibold">Form Update Criteria</h1>
        <input name="criteria"
            type="text"
            ...
            value="{{ old('criteria', $criteria->criteria) }}"
        />
        @error('criteria')
        ...
        <input name="value"
            ...
            value="{{ old('value', $criteria->value) }}"
        />
        @error('value')
            <p class="text-red-500 text-sm mb-2">{{ $message }}</p>
        @enderror
