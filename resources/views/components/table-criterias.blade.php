<div class="p-6 mx-6 bg-white overflow-y-scroll h-3/4 mt-5">
    ...
    <div class="overflow-x-auto w-full">
        <table id="table-criterias" class="min-w-full border-2 border-gray-300 text-sm">
                ...
                @foreach ($criterias as $index => $criteria)
                    <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-100' }}">
                        ...
                        <td class="border-2 border-gray-300 px-4 py-2 space-x-2">
                            ...
                            <form action="{{ url('/delete-criteria/' . $criteria->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button
                                    type="submit"
...
@push('scripts')
    ...
    document.addEventListener("DOMContentLoaded", function () {
        new DataTable('#table-criterias', {
            ...
            select: false,
            language: {
                emptyTable: "Data criteria not found"
            }
        });
    });
    </script>
